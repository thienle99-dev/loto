...
async def wait_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not waiting_list and not already_drawn:
        await update.message.reply_text("⚠️ Không tìm thấy số hợp lệ nào để đợi.")
        return
    ...
    if invalid_list:
        msg_parts.append(f"❌ Các số không hợp lệ: " + ", ".join(invalid_list))
